...
import TransactionRow from 'ui/advancedFilter/TransactionRow';
...
const AdvancedFilter = () => {
  ...
  const [ filters, setFilters ] = React.useState<AdvancedFilterParams>(() => {
    ...
    return {
      // transaction_types is removed from filters to disable type filtering
            ...
            <Tr>
              { columnsToShow.map(column => {
                return (
                  ...
                  >
                    { Boolean(column.name) && (
                      <chakra.span mr={ column.id !== 'type' ? 2 : 0 } lineHeight="24px">
                        { column.name }
                      </chakra.span>
                    ) }
                    {/* Only render filter for non-type columns */}
                    { column.id !== 'type' && (
                      <FilterByColumn
                        column={ column.id }
