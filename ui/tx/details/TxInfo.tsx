...
import ShardTxTypeTag from 'ui/shared/tx/ShardTxTypeTag';
...
const TxInfo = ({ data, isLoading, socketStatus }: Props) => {
  ...
  return (
    <>
      { isSdkTx && (
        <Alert status="warning" fontSize="xs" mb={ 6 }>
          <Text as="span" fontSize="xs">
            This transaction originates from the Shardeum SDK layer. RPC methods like{ ' ' }
            ...
            etc. will not return any details. Please use{ ' ' }
            <chakra.span fontWeight={ 600 }>View raw tx</chakra.span>{ ' ' }
            below to verify its details via the Shardeum SDK REST API.
          </Text>
        </Alert>
      ) }
      ...
      !config.UI.views.tx.hiddenFields?.L1_status && (
        ...
        <DetailsInfoItem.Value columnGap={ 3 }>
          ...
          { isSdkTx && data.cosmos_data?.original_from_address && (
            <Text color="text_secondary" fontSize="xs" width="100%" mt={ -3 } pl={ 7 }>
