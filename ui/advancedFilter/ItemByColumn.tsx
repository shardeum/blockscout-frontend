...
const ItemByColumn = ({ item, column, isLoading, filters }: Props) => {
  switch (column) {
    case 'tx_hash':
    ...
    case 'type': {
      ...
      const { rawInput, isLoading: isRawInputLoading, isStake, isUnstake } = useTransactionRawInput(item.hash);
      
      // When filtering by stake/unstake, maintain consistent labeling
      ...
      if (((isStakeFilter && isStake) || (isUnstakeFilter && isUnstake)) && rawInput) {
        // When filtering specifically for stake/unstake, use the appropriate tag color and label
        return <ShardTxType types={[item.type as any]} input={rawInput} isLoading={isLoading || isRawInputLoading} />;
      }
      
      // Display Shardeum-specific transaction tags when we have raw_input
      if (rawInput) {
        return <ShardTxType types={[item.type as any]} input={rawInput} isLoading={isLoading || isRawInputLoading} />;
      }
      
      // Fall back to standard transaction type display
      const type = ADVANCED_FILTER_TYPES.find(t => t.id === item.type);
      if (!type) {
        return null;
      }
      return <Tag isLoading={isLoading || isRawInputLoading}>{ type.name }</Tag>;
    }
    case 'method':
